...
function apiPath(request) {
  ...
  return pathname
    .replace(/^\/\.netlify\/functions\/items/, '')
    .replace(/^\/api\/items/, '') || '/'
}

function validItem(item) {
  return Object.values(ITEM_TYPE).includes(item.type)
...
function normalizedItem(item) {
  return {
    name: item.name.trim(), type: item.type, place: item.place.trim(), date: item.date || null,
    count: item.count ?? null, status: item.status, image: item.image || null,
    lastDate: item.lastDate || null, cycle: item.cycle ?? null, threshold: item.threshold ?? null,
  }
}
...
export default async (request) => {
  ...
  try {
    ...
    if (request.method === 'POST' && path === '/') {
      ...
      return json({ State: 'SUCCESS', StateCode: 200, Data: {
        PageNum: pageNum, PageSize: pageSize, TotalCount: totalCount,
        TotalPage: Math.ceil(totalCount / pageSize), List: items,
      } })
    }

    if (request.method === 'POST' && path === '/create') {
      const input = await request.json()
      if (!validItem(input)) return json({ message: '品項資料格式不正確' }, 400)
    ...
    if (request.method === 'POST' && useOneMatch) {
      ...
      if (!item) return json({ message: '找不到品項' }, 404)
      if (item.type !== ITEM_TYPE.STOCK) return json({ message: '只有庫存品項可以使用一個' }, 400)
    ...
    if (request.method === 'POST' && maintainMatch) {
      ...
      const item = await findItem(db, id)
      if (!item) return json({ message: '找不到品項' }, 404)
      if (item.type !== ITEM_TYPE.MAINTENANCE) return json({ message: '只有維護品項可以完成維護' }, 400)
      const rows = await db.sql`
    ...
    if (request.method === 'POST' && restockMatch) {
      ...
      const item = await findItem(db, id)
      if (!item) return json({ message: '找不到品項' }, 404)
      if (item.type !== ITEM_TYPE.STOCK) return json({ message: '只有庫存品項可以補貨' }, 400)
      const rows = await db.sql`
    ...
    if (request.method === 'PUT' && idMatch) {
      ...
      const rows = await db.sql`
        ...
        WHERE id = ${id} RETURNING id
      `
      if (!rows[0]) return json({ message: '找不到品項' }, 404)
      await refreshMaintenanceStatuses(db)
