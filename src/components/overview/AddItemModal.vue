        ...
        <header class="flex justify-between">
          <div>
            ...
            <p class="m-0 text-sm text-[#747a77]">
              {{ isEditing ? '更新物品與提醒設定' : '建立維護提醒或庫存提醒' }}
            </p>
          </div>
          <button
        ...
        <div class="my-[21px] mb-[25px] grid grid-cols-2 gap-[17px] max-[700px]:grid-cols-1">
          <button type="button" :class="typeButtonClass(ITEM_TYPE.MAINTENANCE)" @click="form.type = ITEM_TYPE.MAINTENANCE">
            ...
            ><span
              ...
              ><small class="mt-[3px] block text-[13px] text-[#68706c]">依照週期提醒保養、清潔或充電</small></span
          ...
          <button type="button" :class="typeButtonClass(ITEM_TYPE.STOCK)" @click="form.type = ITEM_TYPE.STOCK">
            ...
            ><span
              ...
              ><small class="mt-[3px] block text-[13px] text-[#68706c]">數量不足時提醒補貨</small></span
        ...
        <form @submit.prevent="submitItem">
          <div class="grid grid-cols-[280px_1fr] gap-[22px] max-[700px]:grid-cols-1">
            ...
            >
              ...
              <img v-if="imagePreview" class="h-full w-full object-cover" :src="imagePreview" alt="物品預覽" />
              <template v-else
                ><span class="text-[62px] text-[#087d4b]">▧</span><b class="mt-2 mb-[3px] text-lg">上傳物品照片</b
            ...
            >
              <label
                ><span>品名 <em>*</em></span
                ><input v-model.trim="form.name" required :placeholder="form.type === ITEM_TYPE.MAINTENANCE ? '例如：櫃檯平板' : '例如：拋棄式牙刷'"
              /></label>
              <label><span>位置</span><input v-model.trim="form.place" placeholder="例如：一樓櫃檯" /></label>

              <template v-if="form.type === ITEM_TYPE.MAINTENANCE">
                <label
                  ><span>最近維護日期 <em>*</em></span
          ...
          <div v-else class="mt-[21px] flex items-center gap-3.5 rounded-[9px] border border-[#f5d69f] bg-[#fff7e9] px-[18px] py-3.5 text-[#d97800]">
            <span class="text-[26px]">◇</span>
            <div>
              <b class="block">庫存低於 {{ form.threshold }} 時將自動提醒</b
              ><small class="mt-0.5 block text-[13px] text-[#606864]">目前庫存已低於提醒數量，建立後會立即通知</small>
            </div>
          </div>
          ...
          >
            ...
            <button type="submit" class="border-0 bg-[#087d4b] !text-white disabled:opacity-65" :disabled="submitting">
              {{ submitting ? '儲存中…' : isEditing ? '儲存修改' : '建立物品' }}
            </button>
          </footer>
        </form>
      </section>
