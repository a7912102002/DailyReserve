<template>
  <div class="overview-page">
    ...
    <section class="items-card">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              ...
              <td>
                <div class="maintenance-cell">
                  <span>{{ item.date ?? '-' }}</span
                  ><button
                    v-if="item.type === ITEM_TYPE.MAINTENANCE && [ITEM_STATUS.EXPIRING_SOON, ITEM_STATUS.EXPIRED].includes(item.status)"
                  ...
                  >
                    已維護</button
                  ><button
                    v-if="item.type === ITEM_TYPE.STOCK && [ITEM_STATUS.LOW_STOCK, ITEM_STATUS.OUT_OF_STOCK].includes(item.status)"
                    class="maintained-button"
                    type="button"
            ...
            <tr v-else-if="!filteredItems.length">
              <td colspan="7" class="empty">找不到符合條件的物品</td>
            </tr>
          </tbody>
        </table>
      </div>
      <footer class="table-footer">
        ...
        <div class="pagination">
          ...
          <button v-for="n in totalPage" :key="n" :class="{ current: page === n }" @click="goToPage(n)">
            {{ n }}
          </button>
      ...
      <div v-if="itemToUse" class="fixed inset-0 z-120 grid place-items-center bg-black/45 p-4" @click.self="itemToUse = null">
        <section class="w-full max-w-[410px] rounded-2xl bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="use-one-title">
          ...
          <p class="my-4 text-[#555d59]">
            ...
            <b class="text-[#ef7300]">{{ itemToUse.count }}</b> 扣為 <b class="text-[#ef7300]">{{ itemToUse.count - 1 }}</b
            >。
          </p>
          ...
          <footer class="flex justify-end gap-3 [&_button]:h-10 [&_button]:min-w-24 [&_button]:cursor-pointer [&_button]:rounded-lg">
            ...
            <button class="border-0 bg-[#087d4b] !text-white disabled:opacity-60" type="button" :disabled="usingItem" @click="useOneItem">
              {{ usingItem ? '處理中…' : '確定' }}
            </button>
          </footer>
        </section>
      ...
      <div v-if="itemToRestock" class="fixed inset-0 z-120 grid place-items-center bg-black/45 p-4" @click.self="itemToRestock = null">
        <section class="w-full max-w-[410px] rounded-2xl bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="restock-title">
          ...
          <p class="mb-4 text-center text-sm text-[#68706c]">
            目前 {{ itemToRestock.count ?? 0 }} 個，補貨後 {{ (itemToRestock.count ?? 0) + (Number(restockAmount) || 0) }} 個
          </p>
    ...
    <Teleport to="body">
      ...
      >
        <img class="max-h-[90vh] max-w-[90vw] rounded-xl object-contain shadow-2xl" :src="previewImage" alt="物品放大圖片" />
        <button
...
async function loadItems() {
  ...
  try {
    ...
    console.error(error)
    loadError.value = '無法取得物品資料，請確認 API 是否已啟動'
  } finally {
    loading.value = false
  }
}
